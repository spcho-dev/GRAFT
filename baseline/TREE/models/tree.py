...
class TREE(BaseModel):
    ...
    def build(self):
        ...
        # This layer will project input features (e.g., from 4 to 64)
        self.feature_projection = Dense(self.config.d_model, name='feature_projection')
        # ================================
        ...
        for g in range(self.config.n_graphs):
            ...
            # --- OLD CODE (for pan-cancer datasets) ---
            ...
            # --- NEW, CORRECTED CODE (for specific-cancer datasets) ---
            # 1. Apply the projection to the raw features first.
            projected_features = self.feature_projection(sub_node_feature[:, g, :, :])

            # 2. Now pass the correctly-sized features into the lambda function.
            out = Lambda(lambda x: self.get_node_feature(x[0], x[1]), name=f'get_node_feature_{g}')([projected_features, centr_encoding])
            # ========================================
