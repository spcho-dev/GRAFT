...
class graph_inception(torch.nn.Module):
    ...
    def forward(self, l_feat, r_feat, network, weight1, weight2):
        ...
        if isinstance(network, torch.Tensor):
            ...
            network_l, network_r = sparse_to_matrix(network, l_feat.device)
...
class Net(torch.nn.Module):
    def __init__(self,l_feat, r_feat, network1, network2, hop, inputdims, hiddendims, outputdims, edge_index, edge_index1):
        ...
        self.lin2 = Linear(2, inputdims)    # 팡현 수정
    ...
    def forward(self):
        ...
        # reduced_r_feat = self.reduce_feat_dim(self.r_feat[0])
        # x = self.gcn1(self.l_feat[0], reduced_r_feat, self.network1[0], self.lin3, self.lin4, self.lin5, self.lin6)
...
def load_data(path, device):
    ...
    # feat2, feat3, feat4_exp are duplicates in original code for simplicity
    l_feature.append(feat1)
    l_feature.append(feat1.clone())
    l_feature.append(feat1.clone())
    l_feature.append(feat1.clone())

    r_feature = []
    feat4 = pd.read_csv(path + "P.feat-final.csv", sep=",").values[:, 1:]
    feat4 = torch.Tensor(feat4).to(device)
    feat5 = pd.read_csv(path + "O.feat-final.csv", sep=",").values[:, 1:]
    feat5 = torch.Tensor(feat5).to(device)
    feat6 = pd.read_csv(path + "P.feat-final.csv", sep=",").values[:, 1:]
