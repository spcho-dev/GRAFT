...
**Generating the Data:**
These two files (`node_embeddings.csv` and `attn_info.pt`) can be generated by running the `save_visualization_data.py` script, which utilizes the modified model definition in `model_with_outputs.py` and helper functions in `utils.py`. You need to specify the fold you want to capture data from (the script is set up to capture attention from Fold 1 and embeddings from Fold 10 by default, but can be modified).

Example command to generate the data using the STRING network for pan-cancer (this will create `attn_info_fold1.pt` and `node_embeddings_fold10.csv`):
```bash
