...
* **Input Gene List:**
    * `./node_prediction/predicted_top_genes.txt`
...
* `./node_prediction/node_embeddings.csv`: Contains node embeddings generated by the model, along with true labels and predicted probabilities. Used for UMAP and KDE plots.

* `./node_prediction/attn_info.pt`: Contains model-generated attention weights, along with test indices and true labels. Used for attention visualization scripts (`visualize_average_attention_weight.py`, `visualize_attention_clustermap.py`).

* `./node_prediction/prediction_genes_STRING/` & `./node_prediction/prediction_genes_CPDB/`: Directories containing prediction scores per gene for each fold (`predictions_fold_*.txt`). Used for the gene category plot.

* `./node_prediction/predicted_top_genes.txt`: A list of the top 500 predicted genes with the highest probabilities, filtered to exclude known driver genes (where Label=1). This file is used as input for the Enrichment Analysis Pipeline (specifically `enrichment_analysis_3_gprofiler.py`) to analyze novel candidate genes.

**Note on Reproducibility:** To ensure the reproducibility of the experiments presented in the paper, **all the files listed above are already included in the directory**. The subsequent **Preprocessing and Analysis Pipeline** utilizes these pre-computed files directly.

**Generating the Data:**
These files (`node_embeddings.csv` and `attn_info.pt`) and the prediction results (`predictions_fold_*.txt` and `predicted_top_genes.txt`) can be generated by running the `save_visualization_data.py` script, which utilizes the modified model definition in `model_with_outputs.py` and helper functions in `utils.py`..
...
3.  **Gene Category Analysis and Visualization**
    * **Potential Driver Gene Filtering:** (`filtering_ncg_potential_genes.py`)
       ...
       * **Output:** `node_prediction/potential_driver_genes_ncg7.2.txt` - A list of potential novel driver candidates.
