...
with torch.no_grad():
    ...
    # Capture gene prediction probability
    with open(f'../Data/{args.dataset}/feature_genename.txt') as f:
        gene_names = [line.strip() for line in f.readlines()]
        assert len(gene_names) == final_probs.shape[0], "The number of genetic names and the number of predictions do not match."
...
# 3. Save Gene Predictions
if gene_preds is not None:
    output_dir = output_base_dir
    os.makedirs(f'{output_dir}/prediction_genes_{args.dataset}', exist_ok=True)
    print(f"\n--- Saving gene predictions for '{output_dir}/prediction_genes_{args.dataset}' ---")
    np.savetxt(
        f'./{output_dir}/prediction_genes_{args.dataset}/predictions_fold_{fold_index_to_run+1}.txt',
        gene_preds,
        fmt='%s\t%.7f\t%.1f',
        delimiter='\t',
    ...
    print("Gene predictions saved.")
    
    df_preds = pd.DataFrame(gene_preds, columns=['Gene', 'Predicted', 'Label'])
    df_preds['Predicted'] = df_preds['Predicted'].astype(float)
    df_preds['Label'] = df_preds['Label'].astype(float)
    df_sorted = df_preds.sort_values(by='Predicted', ascending=False)
    df_top500 = df_sorted.iloc[:500]
    df_filtered = df_top500[df_top500['Label'] != 1.0]
    top_genes = df_filtered['Gene']
    top_genes_path = f'{output_base_dir}/predicted_top_genes.txt'
    top_genes.to_csv(top_genes_path, index=False, header=False)
    
    print(f"Top genes list (Top 500 -> Filter Label!=1) saved to '{top_genes_path}'")
    print(f" - Original Top 500 count: 500")
    print(f" - Filtered count (Label!=1): {len(top_genes)}")
    print(f"Top genes list saved to '{top_genes_path}'")
